...
const ResultCard = ({ elem, isLoading = false, heightClass = 'h-[280px]' }) => {
  ...
  }, [isLoading, elem?.id, elem?.src, elem?.thumbnail]);
  ...
  return (
    <article
      ...
      className={`result-card-surface group relative mb-4 break-inside-avoid overflow-hidden rounded-2xl transition-all duration-200 sm:mb-5 lg:mb-6 ${isLoading ? '' : 'cursor-pointer'}`}
    >
      <div className={`relative w-full overflow-hidden rounded-2xl ${heightClass} blur-load ${isMediaLoaded ? 'loaded' : ''} ${isLoading ? 'is-skeleton' : ''}`}>
        {!isLoading &&
          (isVideo || isGifMp4 ? (
            <video
              src={elem.src}
